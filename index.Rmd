...
*Some data points from the CDC 2017 Survey came in the form of a range rather than a single value. In these cases, we represented the range as the mean of its lower and upper limit*

More information about what the data represents or how it was collected by these sources can be found through the above hyperlinks. Further information also provides further context for what the variables in our upcoming visualizations mean:
...
To do this, we plot health outcomes by state and color by the type of state education mandate that exists. The user can select their inputs to explore how outcomes vary by state, and how outcomes may be related to the state mandate. 
...
Users can plot the type of health education mandate on the x-axis of the scatterplot to view how mandates are related to metrics of what actually happens in schools. 
...
This bar plot uses the same x and y variables as the above scatterplot to display the data in a new way. We think this is useful to pick up on different trends. 
...
To investigate this idea, we’ve plotted school policies and teacher practices against health outcomes and draw a correlation line between the two. 
...
This slight correlation is particularly interesting in the context of an earlier question: are stricter state mandates effective in improving student health outcomes? Though there seems to exist just a mild correlation between what teachers actually teach and student outcomes, there was even less correlation between related statewide sex ed mandates and these same student outcomes, suggesting that what teacher's actually teach is more influential on student health outcomes than what the state mandates.
